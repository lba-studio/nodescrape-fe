...
const NewsScoreCard: React.FC<
  NewsScoreCardProps & WithStyles<typeof styles>
> = (props) => {
    ...
    <>
      <div style={{ maxWidth: "100%" }}>
        <Card className={classes.root}>
          <CardActionArea
            onClick={() => window.open(newsSourceScore.url, "_blank")}
          >
            <CardHeader
              avatar={
                <Avatar
                  style={{
                    backgroundColor: computeColorHex(newsSourceScore.score),
                  }}
                >
                  {position}
                </Avatar>
            ...
            />
            <CardContent>
              <Typography>Score: {newsSourceScore.score.toFixed(3)}</Typography>
            </CardContent>
          </CardActionArea>
          <Divider />
      ...
      <Dialog onClose={() => setDialogOpen(false)} open={dialogOpen}>
        ...
        <DialogContent>
          ...
          <Typography variant="subtitle2">
            Last updated:{" "}
            {getTimePassedString(new Date(newsSourceScore.lastUpdatedMs))}
          </Typography>
        </DialogContent>
        <DialogActions>
